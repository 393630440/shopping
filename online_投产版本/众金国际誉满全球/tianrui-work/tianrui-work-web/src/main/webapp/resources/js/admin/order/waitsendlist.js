...
function init(pageNo) {
	$.ajax({
		url : "/admin/shop/order/querywaitsendlist",
		type : "POST",
		data : {
			"pageNo" : pageNo,
			"pageSize" : 10,
...
function innerHTML(data, pageNo) {
	...
	for (var a = 0; a < data.length; a++) {
		...
		html += " class=\"am-btn am-btn-default am-btn-xs am-text-danger am-round\"><span class=\"am-icon-save\"></span>发货";
...
function auditpage(orderId) {
	window.location.href = "/admin/shop/order/sendpage?orderId=" + orderId;
}
